<form action="{{ route('yandas.update', $item->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group mb-3">
        ...
        <select class="form-control" id="Divisi" name="Divisi" required>
            <option value="Sosial" {{ $item->Divisi == 'Sosial' ? 'selected' : '' }}>Sosial</option>
            <option value="Kesehatan" {{ $item->Divisi == 'Kesehatan' ? 'selected' : '' }}>Kesehatan</option>
            <option value="Pendidikan" {{ $item->Divisi == 'Pendidikan' ? 'selected' : '' }}>Pendidikan</option>
        </select>
    </div>
    <div class="form-group mb-3">
